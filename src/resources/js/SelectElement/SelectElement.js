...
let SelectElementError = (
    await import(pathResourcesJs + "/SelectElement/errors/SelectElementError.js")
).default;

let { ServerRequests } = (
    await import(pathNodeModules + "/node_modules/vadimcontenthunter-server-requests/src/ServerRequests.js")
);

let { JsonRpcRequestClient } = (
    await import(pathNodeModules + "/node_modules/vadimcontenthunter-json-rpc-client/src/JsonRpcRequestClient.js")
);

let { serverRequestModule } = (
    await import(pathResourcesJs + "/MainLibrary/MainLibrary.js")
);

let { Notification } = (
    await import(pathResourcesJs + "/Notification/Notification.js")
);
...
export class SelectElement {
    ...
    constructor({
        ...
        this.mainItem =
            this.selectElement.querySelector(".item-selected") ?? null;
    ...
    set selectElement(value) {
        if (!(value instanceof HTMLElement)) {
            throw new SelectElementError(
                "selectElement element must be a DOM element (HTMLElement)."
            );
        }
    ...
    set mainItem(value) {
        if (!(value instanceof HTMLElement)) {
            throw new SelectElementError(
                "mainItem element must be a DOM element (HTMLElement)."
            );
        }
        this.#mainItem = value;
    }
    ...
    set subMenu(value) {
        if (!(value instanceof HTMLMenuElement)) {
            throw new SelectElementError(
                "selectElement element must be a DOM element (HTMLMenuElement)."
            );
        }
        this.#subMenu = value;
    }
    ...
    get subMenu() {
        if (!(this.#subMenu instanceof HTMLMenuElement)) {
            throw new SelectElementError(
                "subMenu element must be a DOM element (HTMLMenuElement)."
            );
        }
    ...
    set serverRequest(value) {
        if (!(value instanceof ServerRequests)) {
            throw new SelectElementError(
                "value element must be a ServerRequests class."
            );
        }
    ...
    get serverRequest() {
        if (!(this.#serverRequest instanceof ServerRequests)) {
            throw new SelectElementError(
                "#serverRequest must be a ServerRequests class."
            );
        }
    ...
    set notification(value) {
        if (!(value instanceof Notification)) {
            throw new SelectElementError(
                "value значение должно иметь родительский класс Notification."
            );
        }
    ...
    get notification() {
        if (!(this.#notification instanceof Notification)) {
            throw new SelectElementError(
                "#notification поле должно иметь родительский класс Notification."
            );
        }
    ...
    getPageId() {
        let input = this.selectElement.querySelector(
            'data input[name="page_id"]'
        );
    ...
    setPageId(value) {
        if (typeof value === "number") {
            throw new SelectElementError(
                "value для setPageId должно быть number."
            );
        }

        let input =
            this.selectElement.querySelector('data input[name="page_id"]') ??
            null;
        if (input instanceof HTMLInputElement) {
        ...
        inputs.forEach((input) => {
            if (input instanceof HTMLInputElement) {
                ...
                if (typeof inputName !== "string" || inputName === "") {
                    throw new SelectElementError(
                        "У input должен быть указан name."
                    );
                }
    ...
    processingData(requestQuery) {
        return JSON.stringify(
            new JsonRpcRequestClient("response", requestQuery)
        );
    }
    ...
    actionItemForSubMenu(item) {
        if (!(item instanceof HTMLElement)) {
            throw new SelectElementError(
                "item для actionItemForSubMenu должно быть HTMLElement."
            );
        }

        if (!item.hasAttribute("value")) {
            throw new SelectElementError(
                "item для actionItemForSubMenu должен иметь атрибут value."
            );
        }
    ...
    hasActivatedDisplaySync() {
        ...
        if (
            syncElement instanceof HTMLElement &&
